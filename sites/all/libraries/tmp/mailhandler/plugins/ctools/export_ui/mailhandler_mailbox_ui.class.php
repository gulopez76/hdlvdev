...
class mailhandler_mailbox_ui extends ctools_export_ui {
  ...
  function edit_form(&$form, &$form_state) {
    ...
    $form['connection']['settings']['port'] = array(
      ...
      '#element_validate' => array('_mailhandler_element_validate_integer_positive'),
      '#ahah' => $ahah_settings,
    );
  ...
  function edit_form_submit(&$form, &$form_state) {
    ...
    if (module_exists('mailhandler_default')) {
      drupal_set_message(t("Now that you've created a mailbox, send it a test email and <a href='@import-page'>create a source node</a> to start importing messages.", array('@import-page' => url('node/add/mailhandler-source'))));
    }
    else {
      drupal_set_message(t("Now that you've created a mailbox, you'll need to <a href='@importer-add'>create a Feeds importer</a> or <a href='@import-page'>run an existing importer</a>. Consider <a href='@module-page'>enabling the Mailhandler quick-start</a> module.", array('@importer-add' => url(MAILHANDLER_MENU_PREFIX . '/feeds/add'), '@import-page' => url('import'), '@module-page' => url(MAILHANDLER_MENU_PREFIX . '/modules'))));
    }
  }
