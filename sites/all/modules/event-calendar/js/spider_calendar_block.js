$(document).ready(function() {
  $("a[class^='ahref_block_']").click(function(event) {
...
function spider_calendar_update_ajax_data_block(event) {
  ...
  if (block_url) {
    $.post(
      ...
      function (data) {
        $('#block-spider_calendar-' + block_id).html($(data).find('#block-spider_calendar-' + block_id).html());
        $('.ahref_block_' + block_id).click(function(event) {
            spider_calendar_update_ajax_data_block(event);
        });
      }
    );
