/**
 * Show block calendar events titles in cloud.
 */
function spider_calendar_show_titles_list(ev, text) {
  spider_calendar_get_cursor_xy(ev);
  var str = String(Drupal.settings.spider_calendar.site_url);
  var url;
  if (str.indexOf(',') >= 0) {
    url = str.substring(0, str.indexOf(','));
  }
  else {
    url = Drupal.settings.spider_calendar.site_url;
  }
  document.getElementById('spiderCalendarTitlesList').innerHTML = '<table style="border-collapse:separate;" cellpadding="0" cellspacing="0" border="0" width="100%"><tr><td id="spider_calendar_src1" style="background: url(\'' + url + '/TitleListBg1.png\') no-repeat scroll 0 0 transparent; height: 65px; margin: 0; padding: 0;">&nbsp;</td></tr><tr><td id="spider_calendar_src2" style="background: url(\'' + url + '/TitleListBg2.png\') repeat-y scroll 0 0 transparent; margin: 0; padding: 0 20px 0 20px;">' + text + '</td></tr><tr><td id="spider_calendar_src3" style="background: url(\'' + url + '/TitleListBg3.png\') no-repeat scroll 0 0 transparent; height: 32px; margin: 0; padding: 0;">&nbsp;</td></tr>';
